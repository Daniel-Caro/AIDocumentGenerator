...
public interface DocumentRepository extends JpaRepository<Document, Long> {
	...
	@Query("SELECT d FROM Document d WHERE (:title is null or d.title = :title) and (:date is null or d.date = :date) and (:authors is null or d.authors = :authors) and (:color is null or d.color = :color) and (:userId is null or d.user.id = :userId) and (:urlView is null or d.urlView = :urlView) and (:urlEdit is null or d.urlEdit = :urlEdit)")
